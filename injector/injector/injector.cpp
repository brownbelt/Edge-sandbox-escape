...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if (option == 1) {
		...
		getline(file, line, '\x9f');
		file.close();
		int i = line.size();
		sc = new char[line.size()+1];
		sc = (char*)line.c_str();
		printf("%d", line.size() + 1);
		hProcess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, PID);
		if (hProcess == NULL)
		{
			wcerr << "Cannot open process: " << PID << " error: " << GetLastError() << endl;
		...
		hAllocatedMem = VirtualAllocEx(hProcess, NULL,4096, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
		if (hAllocatedMem == NULL)
		{
			wcerr << "Cannot allocate memory in " << PID << ", error: " << GetLastError() << endl;
			exit(1);
		}
		wcout << "Memory allocation: OK" << hAllocatedMem << endl;

		if (!WriteProcessMemory(hProcess, hAllocatedMem, sc, 4096, NULL))
		{
